...
def n_estimators_objective(trial, X, y):
    ...
    learning_rates = list(
        ...
        * np.float_power(1 - 7e-4, np.arange(lgbm_params["n_estimators"]))
    )
    ...
    eval_df = model.eval_lgbm_cv(
        ...
        callbacks=[
            optuna.integration.LightGBMPruningCallback(
                trial, "binary_logloss", valid_name="validation"
            ),
...
def n_estimators_grid(n_trials=20, outdir="."):
    ...
    sql_file = (
        f'sqlite:///{str(utils.WORKING_DIR / outdir / "lgbm_n_estimators_grid.db")}'
    )
    ...
    study = optuna.create_study(
        ...
        sampler=optuna.samplers.GridSampler(
            search_space={
                "n_estimators": np.geomspace(1, 1000, num=n_trials, dtype=int).tolist()
            }
        ),
    )
...
def early_stopping_grid(n_trials=21, outdir="."):
    ...
    sql_file = (
        f'sqlite:///{str(utils.WORKING_DIR / outdir / "lgbm_early_stopping_grid.db")}'
    )
    ...
    study = optuna.create_study(
        ...
        sampler=optuna.samplers.GridSampler(
            search_space={
                "learning_rate": np.geomspace(1e-5, 1.0, num=n_trials).tolist()
            }
        ),
    )
...
def stage0_objective(trial, X, y):
    ...
    lgbm_params = {
        ...
        "learning_rate": 5e-2,  # expect around 2k trials 2-3min, 5k seemed to take 5hrs
...
def cv_best_trial(learning_rate=5e-1, outdir=None):
    ...
    lgbm_params = {
        ...
        "learning_rate": learning_rate,  # expect around 2k trials 2-3min
