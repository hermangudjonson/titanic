...
def n_estimators_objective(trial, X, y):
    ...
    learning_rates = list(
        ...
        * np.float_power(1 - 7e-4, np.arange(xgb_params["n_estimators"]))
    )
...
def n_estimators_grid(n_trials=20, outdir="."):
    ...
    sql_file = (
        f'sqlite:///{str(utils.WORKING_DIR / outdir / "xgb_n_estimators_grid.db")}'
    )
...
def max_depth_grid(outdir="."):
    ...
    # pre-load data for trials
    raw_train_df, target_ds = load_prep.raw_train()
    study.optimize(functools.partial(max_depth_objective, X=raw_train_df, y=target_ds))
    warnings.resetwarnings()
    return study
...
def _cv_results_df(cv_results: dict):
    ...
    cv_results_df = pd.concat(
        [
            pd.concat(
                cv_results["xgb_metrics"].values(), keys=folds, ignore_index=True
            ),
