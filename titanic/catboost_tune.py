...
def _cv_results_df(cv_results: dict):
    ...
    cv_results_df = pd.concat(
        [
            pd.concat(
                cv_results["ctb_metrics"].values(), keys=folds, ignore_index=True
            ),
...
def n_estimators_grid(n_trials=20, outdir="."):
    ...
    sql_file = (
        f'sqlite:///{str(utils.WORKING_DIR / outdir / "ctb_n_estimators_grid.db")}'
    )
    ...
    study = optuna.create_study(
        ...
        sampler=optuna.samplers.GridSampler(
            search_space={
                "n_estimators": np.geomspace(1, 10, num=n_trials, dtype=int).tolist()
            }
        ),
    )
...
def early_stopping_grid(n_trials=21, outdir="."):
    ...
    sql_file = (
        f'sqlite:///{str(utils.WORKING_DIR / outdir / "ctb_early_stopping_grid.db")}'
    )
