...
def example_submission(input_dir: Path | str = INPUT_DIR):
    ...
    gender_ds = pd.read_csv(
        input_dir / "gender_submission.csv", index_col="PassengerId"
    ).squeeze()
    return gender_ds
...
class TicketTransformer(BaseEstimator, TransformerMixin):
    ...
    def _make_line_unique(self, df):
        ...
        df.loc[df.Ticket == "LINE", "Ticket"] = [
            f"LINE{i}" for i in range(self.line_id, self.line_id + K)
        ]
        self.line_id += K
    ...
    def transform(self, df):
        ...
        # ticket grouping
        # create a feature indicating the survival rate of other members of a
        # ticket group (in training)
        ...
        t_count_long = df[["Survived", "Ticket"]].merge(
            self.t_count, how="left", left_on="Ticket", right_index=True
        )
        ...
        t_count_long[
            "nt_survived"
        ] = t_count_long.t_survived - t_count_long.Survived.fillna(0)
...
def _transform_ticket(df):
    ...
    # make LINE unique
    df.loc[df.Ticket == "LINE", "Ticket"] = [
        f"LINE{i}" for i in range(sum(df.Ticket == "LINE"))
    ]
    ...
    # long form, create column entry for each passenger
    t_count_long = df[["Survived", "Ticket"]].merge(
        t_count, left_on="Ticket", right_index=True
    )
    # adjust so passenger's own counts don't contribute
    t_count_long[
        "nt_survived"
